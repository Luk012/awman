...
@Config
@Autonomous(group = "Auto" , name = "TestExtendo")
...
// Third cycle scoring positions
        ...
        STROBOT status = STROBOT.EXTENDO_EXTENDO;
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            switch (status) {
                ...
                case EXTENDO_EXTENDO: {
                    if (prepare_collect.seconds() > 3)
                    { if(extendo.CS != extendoController.extendoStatus.EXTENDED)
                    {
                        extendo.CS = extendoController.extendoStatus.EXTENDED;
                        extendo_timer.reset();
                        status = STROBOT.EXTENDO_RETRACT;
                    }}
                    break;
                }

                case EXTENDO_RETRACT:
                {
                    if(extendo_timer.seconds() > 3)
                    {
                        extendo.CS = extendoController.extendoStatus.RETRACTED;
                        prepare_collect.reset();
                        status = STROBOT.EXTENDO_EXTENDO;
                    }
                    break;
                }
